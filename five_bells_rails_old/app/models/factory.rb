class Factory < Company
  ...
  def produce(quantity=1)
    ...
    # TODO: calculate if we can actually afford all the components...
    required_deposit = 0
    ...
    # first acquire all the required components (in bulk) to see if we can actually acquire
  ...
  def evaluate(cycle)
    ...
    buyer = region.markets.buys(product_name).shuffle.first
    buyer ||= region.factories.produces(recipe.parent.product_name).shuffle.first

    unless buyer.nil? && factory.nil?
      ...
      if (employees.size == 0 || account.deposit > 3 * salaries_paid)
        region.labour_buyer.hire(offered_salary, self)
        offered_salary += 1
      end
  ...
  def set_name
    self.name ||= "#{region.name.gsub(/\s/, '')}-Factory-#{id}"
  end
end
