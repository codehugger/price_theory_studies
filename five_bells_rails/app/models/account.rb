class Account < ApplicationRecord
  ...
  has_many :debit_transfers, foreign_key: 'credit_id', class_name: 'Transfer'
  has_many :credit_transfers, foreign_key: 'debit_id', class_name: 'Transfer'

  def transfers
    Transfer.where("credit_id = ? OR debit_id = ?", id, id).order(created_at: :desc)
  end
