class Market < Company
  ...
  def increase_spread(amount)
    ...
    spread = max_spread if spread > max_spread
    update!(spread: spread, sell_price: self.bid_price * spread)
  end

  def decrease_spread(amount)
    spread = [self.spread + amount, self.min_spread].min
    spread = min_spread if spread < 0
    update!(spread: spread, sell_price: self.bid_price * spread)
  end
  ...
  def record_stats_and_reset
    record_stats
    reset_internals
  end

  def record_stats
    statistics.find_by(name: "market:#{name}:spread").record!(self.spread)
    statistics.find_by(name: "market:#{name}:bid_price").record!(self.bid_price)
    statistics.find_by(name: "market:#{name}:sell_price").record!(self.sell_price)
    statistics.find_by(name: "market:#{name}:offered_salary").record!(self.offered_salary)
    statistics.find_by(name: "market:#{name}:salaries_paid").record!(self.salaries_paid)
    statistics.find_by(name: "market:#{name}:inventory_size").record!(self.products.count)
  end
  ...
  def init_statistics
    Statistic.create!(world: world, name: "market:#{name}:spread")
    Statistic.create!(world: world, name: "market:#{name}:bid_price")
    Statistic.create!(world: world, name: "market:#{name}:sell_price")
    Statistic.create!(world: world, name: "market:#{name}:offered_salary")
    Statistic.create!(world: world, name: "market:#{name}:salaries_paid")
    Statistic.create!(world: world, name: "market:#{name}:inventory_size")
  end

  def set_name
    self.name ||= "#{world.name.gsub(/\s/, '')}-Market-#{product.name.gsub(/\s/, '')}-#{id}"
  end
end
